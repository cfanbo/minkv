...
// Hint => bytes
impl From<Hint> for Vec<u8> {
    fn from(val: Hint) -> Self {
        let mut result: Vec<u8> = Vec::with_capacity(28 + val.key_size as usize);
        ...
        result.extend_from_slice(&val.value_pos.to_le_bytes());
        result.extend_from_slice(&val.key);
